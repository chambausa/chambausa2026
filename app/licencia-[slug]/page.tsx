...
export async function generateStaticParams() {
  ...
  const dbSlugs = pages.slice(0, 50).map((page) => ({
    slug: page.slug,
  }))

  // Add JSON-based pages (without 'licencia-' prefix since it's already in the route)
  const jsonSlugs = [
...
async function loadJSONContent(slug: string) {
  console.log('[DEBUG] loadJSONContent called with slug:', slug)

  // Try exact match (slug comes WITHOUT the 'licencia-' prefix)
  if (slug === 'cosmetologia-texas') {
    try {
      const fs = await import('fs/promises')
      const path = await import('path')
      const filePath = path.join(process.cwd(), 'archivos_json', 'cosmetologia-texas.json')
      console.log('[DEBUG] Trying to load JSON from:', filePath)
      
      try {
        await fs.access(filePath)
        console.log('[DEBUG] File exists at path')
      } catch (accessError) {
        console.error('[DEBUG] File does NOT exist at path:', accessError)
        return null
      }

      const fileContent = await fs.readFile(filePath, 'utf-8')
      console.log('[DEBUG] File content loaded, length:', fileContent.length)
      return JSON.parse(fileContent)
    } catch (e) {
      console.error('[DEBUG] Error loading JSON:', e)
      return null
    }
  }
  return null
